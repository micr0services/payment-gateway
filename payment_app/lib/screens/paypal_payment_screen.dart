...
class _PayPalPaymentScreenState extends State<PayPalPaymentScreen> {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Center(
                            child: TextButton(
                              onPressed: () {
                                Navigator.of(context).pushReplacementNamed('/stripe');
                              },
                              child: const Row(
  ...
  Future<void> _processPayment() async {
    ...
    try {
      ...
      final approvalUrl = await paymentProvider.createPayPalPayment(
        amount: _amount,
        currency: _currency,
      );

      if (approvalUrl.isNotEmpty) {
        // Launch the approval URL in external browser
        if (await canLaunchUrl(Uri.parse(approvalUrl))) {
          await launchUrl(Uri.parse(approvalUrl), mode: LaunchMode.externalApplication);
          setState(() {
            _message = 'Redirecting to PayPal checkout...';
          });
        } else {
          setState(() {
            _message = 'Failed to open approval URL';
          });
        }
      } else {
        setState(() {
          _message = 'Failed to create PayPal order';
        });
      }
    } catch (error) {
      setState(() {
